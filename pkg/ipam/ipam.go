...
func (i *Ipam) ExecAdd(conf *NetConf) (*cniv1.Result, error) {
	...
	if conf.IP != "" {
		...
		if err := i.UpdatePool(conf, IPPoolOffsetIgnore, IP_ADD); err != nil {
			return nil, err
		}
	...
	for retry := 0; retry < FindRetryCount; retry++ {
		...
		if err := i.UpdatePool(conf, newOffset, IP_ADD); err != nil {
			klog.Error(err)
			continue
		}
...
func (i *Ipam) ExecCheck(conf *NetConf) error {
	return nil
}
...
func (i *Ipam) ExecDel(containerID string) error {
	...
	for _, item := range ipPools.Items {
		_ = i.UpdatePool(&NetConf{Pool: item.Name, ContainerID: containerID}, 0, IP_DEL)
	}
